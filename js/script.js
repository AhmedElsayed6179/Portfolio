    ...
    toggleBtn.addEventListener('click', () => {
      ...
      if (isExpanded) {
        ...
        hiddenProjects.forEach((card, i) => {
          card.classList.remove('hide-animate');
        ...
        // Done animating after last card finishes
        const lastDelay = (hiddenProjects.length - 1) * 90 + 650;
        setTimeout(() => {
          isAnimating = false;
          AOS.refresh();
        }, lastDelay);
        ...
        // Hide cards in reverse with staggered animation
        const reversed = Array.from(hiddenProjects).reverse();
        reversed.forEach((card, i) => {
          card.classList.remove('show-animate');
          void card.offsetWidth;
          card.style.animationDelay = `${i * 60}ms`;
          card.classList.add('hide-animate');
        });

        const lastDelay = (reversed.length - 1) * 60 + 380;
        setTimeout(() => {
          ...
          isAnimating = false;
          if (visibleCountEl) visibleCountEl.textContent = 6;
